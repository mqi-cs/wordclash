...
export const Keyboard = ({
  ...
  return (
    <div className="mx-auto w-full max-w-[32rem] px-1.5 pb-[max(0.25rem,env(safe-area-inset-bottom))] sm:max-w-sm sm:px-1">
          ...
          {row.map((key) => {
            ...
            return (
              <Button
                ...
                disabled={disabled}
                className={cn(
                  "relative h-11 min-w-0 flex-1 overflow-hidden rounded-xl px-1 font-semibold text-[12px] transition-colors sm:h-11 sm:text-sm",
                  isActionKey
                    ? "max-w-[4.35rem] bg-muted/85 px-1.5 text-[10px] sm:max-w-none sm:flex-[1.35] sm:px-2.5 sm:text-sm"
                    : "max-w-[2.8rem] bg-game-key-bg sm:max-w-none",
